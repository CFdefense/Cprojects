//This file will serve as the main gameplay structure
#include <iostream>
#include <Windows.h>
#include <cmath>
#include <chrono>
...
float characterYpos = 15.0f;
const float defaultXpos = 8.0f;
const float defaultYpos = 15.0f;
float characterAngle = 3.0;
...
//Boolean for manipulating game cycle and Boolean for game menu
bool isPaused = false;
bool inMenu = false;


int play(wstring map)
{
		...
		if(inMenu == false)
			{
				...
				if (characterXpos < 0 || characterXpos >= mapWidth || characterYpos < 0 || characterYpos >= mapHeight)
						{	
							//reset them to starting position and display out of bounds message
							characterXpos = defaultXpos;
							characterYpos = defaultYpos;
						}//if
				...
				{
					inMenu = true; // Toggle the inMenu variable
			...
			else
			{
				...
				// Display the pause screen options
				swprintf_s(screen + screenWidth * (screenHeight / 2 - 1) + screenWidth / 2 - 5, 20, L"  PAUSED  ");
				swprintf_s(screen + screenWidth * (screenHeight / 2) + screenWidth / 2 - 5, 20, L" 1. Unpause ");
				swprintf_s(screen + screenWidth * (screenHeight / 2 + 1) + screenWidth / 2 - 5, 20, L" 2. Quit    ");

				// Display the frame
				screen[screenWidth * screenHeight] = L'\0';
				WriteConsoleOutputCharacterW(console, screen, screenWidth * screenHeight, { 0,0 }, &dwBytesWritten);

				// Check for user input to unpause or quit
				if (GetAsyncKeyState((unsigned short)'1') & 0x8000)
				{
					inMenu = false; // Unpause the game
				}
